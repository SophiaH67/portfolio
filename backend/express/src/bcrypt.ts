...
const getSalt = async () => {
  ...
  if (result) {saltCache = result.value; return saltCache}
...
export const setPasswordHash = async (passwordHash: string) => {
  if(await getPasswordHash()) {
    ...
    const hash = await Value.create({key: 'PASSWORD_HASH', value: passwordHash})
    await hash.save()
  }
}

export const validatePasswordHash = async (passwordHash: string) => 
  safeCompare(passwordHash, await getPasswordHash());

export const hashPassword = async (password: string) => 
  hash(password, await getSalt())
